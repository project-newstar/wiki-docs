...
:::warning
由于编者中文能力较弱，未能全部有效理解下面的内容，故可能存在无法正确排版的情况。若有读者具有超强的阅读理解能力，欢迎对本文进行重构，并联系 <newstar@openctf.net>.
:::
...
因为有实际背景, 在可可液块一栏中只要输入了小于等于 0 这一**不符合现实的量**, 就会出现 hint (就向某些支付漏洞一样)
...
在线网站/脚本解密之后是`the next level is : cocoaButter_star.php`
...
最后一步稍微难点, 需要新生写一个脚本实现 字符串加密后前几位等于'8031b'，以下脚本供参考
...
这里跑出来的结果 `60066549`，那么 payload 可以是
...
需要用 BurpSuite 等方法发包（抓包，然后在数据包里面填上序列化之后的内容，类似 POST）

`$food` 的反序列化后传递给 `$milk` 后，如果 `$milk` 带有 `chocolate`，那么就会被程序抛出异常从而终止反序列化
...
```php
 $a = new test();    //$a 是对象
 print_r($a);        //调用对象可以用print_r()或var_dump()
 echo $a;            //触发
...
简单来说就是这个类的实例被销毁时触发( 执行 )这个函数

也就是我们通过实例化一个正确的 chocolate 类, 等到这个实例被 php 销毁就自然输出我们可能需要的内容了(echo $darkCocoaPowder;)  实际上这个参数的值在 index.php 中抓包或者分析源码也能看出这就是黑可可粉的参数

只要传入`O:9:"ChocoLate":2:{s:8:"username";s:3:"???";s:8:"password";s:3:"???";}` (至少有一个字母大写即可)
输出 `// 黑可可粉 (g): 51540`

参考：

```php
 <?php class chocolate{     public $username='???';  
     public $password='???'; }  
 $c = new chocolate();  
...
正确填写了三个值之后, 任意填一个"糖粉"的值 , 页面会返回 "太苦"
...
希望做题的新生能通过这题感觉到难度在上升 但是都能够通过努力战胜出题人 : )
